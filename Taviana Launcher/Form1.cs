...
namespace Taviana_Launcher
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            if (!hasError)
            {
                text_StatusLabel.Text = "";
                text_StatusLabel.BackColor = Color.Black;
                text_StatusLabel.Left = ((text_StatusLabel.Parent.Width - text_StatusLabel.Width)/2);
                button_Action.Text = "Launch DayZ Revolution";
            }
            else
            {
        ...
        private string ARMAPath(string key) {
            ...
            string subkey = "SOFTWARE\\Wow6432Node\\Bohemia Interactive\\ArmA 2 OA\\BattlEye";
            string keyName = userRoot + "\\" + subkey;

            string mainPath = (String)Registry.GetValue(keyName, "main", "");
            string expPath = (String)Registry.GetValue(keyName, "DATA", "");

            subkey = "SOFTWARE\\Wow6432Node\\Bohemia Interactive\\ArmA 2\\BattlEye";
        ...
        {
            Process.Start("http://iaregameplayer.com/smf/index.php?action=treasury");
        }

        private void button_Action_Click(object sender, EventArgs e)
        {
            if (button_Action.Text == "Launch DayZ Revolution")
            {
                args = "-skipIntro -mod=@Taviana -noSplash -noFilePatching -world=empty -connect=74.91.116.171" + 
                    " -port=2302 \"-mod=" + ARMAPath("arma2path") + 
                    ";expansion;expansion\\beta;expansion\\beta\\expansion;@DayzRevolution\"";
                ProcessStartInfo proc = new ProcessStartInfo();
                //proc.UseShellExecute = true;
                //proc.Verb = "runas";
                proc.WorkingDirectory = ARMAPath("path");
                proc.FileName = ARMAPath("path") + "\\" + "arma2oa.exe";
                proc.Arguments = args;
